...
def _create_registry_api(key, default=M, raise_for_missing=False, registry=None):
    if registry is None:
...
def get_fields(cls, aliased_form=False):
    d = cls if isinstance(cls, dict) else vars(_ensure_type(cls))
    return {
        (
            get_alias(k) if
            k in get_aliased_args(cls)
            and aliased_form
            else k
        ): v for k, v in d.items()
        if (
            k not in IGNORE_ATTRS
...
class ConstructAttributeTree:
    ...
    def __repr__(self):
        ...
        fields_fmt = ', '.join(f'{k}={v!r}' for k, v in get_fields(self).items())
        return cls_name + f'({fields_fmt})'


Struct = port(cs.Struct)
Sequence = port(cs.Sequence)
FocusedSeq = port(cs.FocusedSeq, reserve_args=['parsebuildfrom'])
Union = port(cs.Union, reserve_args=['parsefrom'])
