...
REPEATED_NAME_FMT = "%(name)s[%(size)d]"
REPEATED_MEMBER_NAME_FMT = "%(name)s_%(index)d"
...
class Model:
    ...
    _repeated_name_fmt = None
    _repeated_member_name_fmt = None
    ...
    def _choose_fields(self, settings: SettingsT) -> dict[Any, Field]:
        namespace = set(self.choose_components(**settings))
        fields = {
            name: desc for name, desc in self._fields.items() if name in namespace
        }
        return fields
    ...
    def default(self) -> Any:
        ...
        for name, field in self._fields.items():
            model = field.get_component(self)
    ...
    def get_state(self, empty=MISSING, /, **settings: Any) -> dict:
        fields = self._choose_fields(settings)
        states = {}

        for name, field in fields.items():
            state = field.get_state(self, empty, settings)
            ...
            if state is MISSING:
                ...
                if empty is not MISSING:
                    ...
                    raise ValueError(
                        f"missing required {type(field.component).__name__} "
                        f"value for serializer named {field.component.name!r}"
                    )
    ...
    def read_sequence_state(self, load: collections.abc.Sequence) -> "dict[str, Any]":
        return dict(zip(self._fields, load))
    ...
    def clear(self):
        return self.set_state(dict.fromkeys(self._fields, MISSING))
    ...
    def __getitem__(self, key: Any):
        return self._fields[key].__get__(self, None)
    ...
    def _load_stack(cls, stack, settings: SettingsT):
        ...
        cls._fields = fields = collections.OrderedDict()
...
def repeated(
    ...
    name=None,
    name_fmt=None,
    member_name_fmt=None,
    factory=create_model,
) -> Type[Model]:
    if repeat < 1:
        raise ValueError("dimension size must be at least 1")
    if name_fmt is None:
        name_fmt = REPEATED_NAME_FMT
    if member_name_fmt is None:
        member_name_fmt = REPEATED_MEMBER_NAME_FMT
    if name is None:
        name = cls.__name__
    fmt = {"name": name, "size": repeat}
    components = (
        cls(name=member_name_fmt % {**fmt, "index": i + 1})
        for i in range(repeat)
    )
    return factory(*components, name=name_fmt % fmt)
